...
public class Shockwave extends CustomEnchant implements TriggerOnInteractEnchantment {
    ...
    public void onInteract(PlayerInteractEvent e, int level) {
        ...
        for (Entity entity : entitiesInRadius){
            if (entity instanceof LivingEntity && !EntityClassificationType.isMatchingClassification(entity.getType(), EntityClassificationType.UNALIVE)){
                // Skip knockback if PvP is disabled at the entity's location
                if (!isPvPEnabledAtLocation(entity.getLocation())) continue;
    ...
    private boolean isPvPEnabledAtLocation(Location location) {
        // Use the Towny API to check if PvP is disabled in the area
        try {
            if (TownyAPI.getInstance().isWilderness(location)) return true; // PvP allowed in wilderness
            return TownyAPI.getInstance().getTownBlock(location).getPermissions().pvp;
        } catch (Exception e) {
            return true; // Default to PvP allowed if unsure
        }
    }
}
